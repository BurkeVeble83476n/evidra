...
## Calling The Prescribe Tools

### Recommended default: `prescribe_smart`

Record lightweight intent BEFORE an infrastructure operation that creates, modifies, or deletes resources. Use this tool when you know the tool, operation, and target resource but do not have artifact bytes.
...
### Smart pre-call checklist
- tool/operation must be non-empty.
- resource must identify the target (for example deployment/web).
...
Record full artifact intent BEFORE an infrastructure operation that creates, modifies, or deletes resources. Use this tool when you have the actual artifact bytes and want native detector coverage plus artifact drift tracking.
...
### Full pre-call checklist
- tool/operation must be non-empty.
- raw_artifact must contain the artifact or command content being executed.
- actor.type/actor.id/actor.origin must be present.
- actor.skill_version should be set to this contract version.
- Keep session/operation/trace identifiers stable within one task.
...
### What the prescribe tools return
...
After a prescribe tool returns `ok=true`, execute the infrastructure command normally. Capture:
...
1. Call `prescribe_full` again if you still have artifact bytes, or `prescribe_smart` again if you do not
2. Execute the command
