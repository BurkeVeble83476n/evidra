...
Update the case's `artifact_ref` to point to the new file. The old file stays — it may be referenced by golden files, and removing it breaks reproducibility.
...
Detectors change. A new release might modify how `k8s.privileged_container` is detected, changing the prescribe output for 15 cases. Without golden files, CI passes silently even when behavior changes. With golden files, every change produces a visible diff.

### Implementation

For each case, store **only the contract fields** from the detector output — not the full prescribe JSON. This dramatically reduces golden file churn. A refactor that changes internal field names or adds new informational fields does not break goldens; only actual contract changes do.
...
Including `ground_truth_pattern` in the golden makes debugging immediate: you see expected pattern vs detected tags side by side.

For scenarios, a separate golden:
```
...
for field in risk_level operation_class scope_class resource_count canon_version; do
  expected=$(jq -r ".$field" "cases/$CASE/golden/contract.json")
...
expected_tags=$(jq -r '.risk_tags | sort | join(",")' "cases/$CASE/golden/contract.json")
...
make golden-update-benchmark

# Review the diff
git diff tests/benchmark/cases/*/golden/

# Commit with explanation
git commit -m "update golden: new hostPath detection logic in k8s adapter"
```

This makes detector changes visible at PR time. Reviewer sees: "this PR changes the risk detector and updates 7 golden files" — clear, auditable.

### Contract Fields (stored in golden)
...
### Non-Contract Fields (NOT stored in golden)
...
Storing only 6 contract fields means a detector refactor that changes internal JSON structure but preserves the same risk_level/risk_tags/operation_class output does NOT break any golden file. This is the point.

### Critique: Golden Files Are Maintenance Burden

Every detector change touches golden files. If you have 50 cases, a single detector fix could require updating 20 golden files. This is tedious.

**Mitigation:** `make golden-update-benchmark` regenerates all goldens. The human cost is reviewing the diff, not manually editing files. This is a feature, not a bug — you want to see exactly what changed.

**Risk:** If golden updates become rubber-stamped ("just regenerate and commit"), they lose their value. Counter: CI should also run `validate-source-composition.sh` to ensure the updates are consistent.
...
Every `expected.json` records which Evidra version produced its golden output and risk expectations:
...
This answers: "which version of Evidra produced this golden?" If a case fails CI after an Evidra upgrade, you see immediately: golden was created with v0.5.0, running v0.6.0 now, expected difference.
...
Processing pipeline (pinned to dataset's evidra_version):
  ...
  2. Extract contract fields → golden/contract.json
...
if [ ! -f "$EVIDRA_BIN" ]; then
  ...
  curl -sL "https://github.com/vitas/evidra-benchmark/releases/download/v${DATASET_EVIDRA_VERSION}/evidra_${DATASET_EVIDRA_VERSION}_$(uname -s)_$(uname -m).tar.gz" \
    | tar -xz -C bin/
...
At 50 cases, `upgrade-dataset.sh` takes 10 seconds and the diff fits on one screen. The machinery feels heavy. But version pinning prevents a real problem: a contributor runs `evidra prescribe` with their local Evidra v0.7.0-dev while the dataset is on v0.5.0. Their golden contract.json will have different risk_tags than everyone else's. CI catches this via the version consistency check.
...
Step 6: Create golden output
  → Extract contract fields from prescribe output into golden/contract.json
...
#     golden/            (empty, run evidra prescribe to populate contract.json)
...
for golden in cases/*/golden/contract.json; do
  [ -f "$golden" ] || continue
  # ... golden validation logic from section 5
done
...
# 12. Risk details consistency: golden risk_tags must match risk_details_expected
for case_dir in cases/*/; do
  golden="$case_dir/golden/contract.json"
  expected="$case_dir/expected.json"
  [ -f "$golden" ] && [ -f "$expected" ] || continue

  expected_tags=$(jq -r '(.risk_details_expected // []) | sort | join(",")' "$expected")
  golden_tags=$(jq -r '(.risk_tags // []) | sort | join(",")' "$golden")

  if [ "$expected_tags" != "$golden_tags" ]; then
    case_id=$(basename "$case_dir")
    echo "FAIL: $case_id risk_details_expected ($expected_tags) != golden risk_tags ($golden_tags)"
    CHECKS_FAILED=$((CHECKS_FAILED + 1))
  fi
done
...
echo "  4. Save contract fields to $CASE_DIR/golden/contract.json"
...
When the shell script gets called 50+ times and people request features like auto-detection of category from artifact content, or auto-running prescribe to populate golden. Until then, shell is fine.
...
|----------|---------|---------------|
| Store golden files in git? | Yes (visible in PRs) / No (generated in CI) | **Yes** — PR visibility is the whole point |
...
- Create golden contract.json for each
