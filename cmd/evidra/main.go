...
func cmdScorecard(args []string, stdout, stderr io.Writer) int {
	...
	view := buildScorecardView(sc, signalEntries, *actorFlag, *sessionIDFlag, *periodFlag)
...
func buildScorecardView(sc score.Scorecard, signalEntries []signal.Entry, actorID, sessionID, period string) scorecardView {
	...
	for _, name := range scorecardSignalOrder {
		...
		profile := sc.SignalProfiles[name].Level
		if profile == "" {
			profile = "none"
		}
		rows = append(rows, scorecardSignalRow{
			...
			Rate:    rate,
			Profile: profile,
			Weight:  score.DefaultWeights[name],
		})
	}
