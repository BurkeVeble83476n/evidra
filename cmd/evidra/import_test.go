...
func TestImportCommandWritesPrescribeAndReport(t *testing.T) {
	t.Parallel()
...
func TestImportCommandRejectsInvalidPayload(t *testing.T) {
	t.Parallel()
	...
	if code != 2 {
		t.Fatalf("import invalid payload exit=%d want 2 (stderr=%s)", code, errBuf.String())
	}
